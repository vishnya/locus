...
- Header: "Locus: Work" with tab/dropdown when multiple profiles exist
...
- `LOCUS_PROFILE=work lc status` env var override
...
1. Add config file support (`~/.config/locus/config.json`)
...
```json
// in ~/.config/locus/config.json (or per-profile)
{
  "enrichment": {
