...
from locus.priorities import parse, render, load, save, Priorities, Task, ProjectInfo
...
## Active
- [ ] [Locus] !! fix hammerspoon chord hotkeys
- [ ] [Anki Fox] test screenshot flow

## Up Next
- [ ] [Locus] push updated project model
- [ ] [Locus] test on second device
- [ ] [Anki Fox] review PR
...
def test_parse_active():
    ...
    assert p.active[0].project == "Locus"
...
def test_parse_up_next():
    ...
    assert p.up_next[0].project == "Locus"


def test_parse_projects():
    p = parse(SAMPLE)
    assert len(p.projects) == 2
    assert p.projects[0].name == "Locus"
...
def test_task_to_line():
    t = Task(text="fix bug", project="Locus", done=False)
    assert t.to_line() == "- [ ] [Locus] fix bug"
    t.done = True
...
def test_find_project():
    p = parse(SAMPLE)
    assert p.find_project("locus").name == "Locus"
...
def test_get_project_case_insensitive():
    p = parse(SAMPLE)
    assert p.get_project("locus").name == "Locus"
    assert p.get_project("LOCUS").name == "Locus"
...
def test_project_names():
    p = parse(SAMPLE)
    assert p.project_names() == ["Locus", "Anki Fox"]
...
def test_sync_project_to_obsidian(tmp_path):
    os.environ["LOCUS_VAULT"] = str(tmp_path / "PRIORITIES.md")
    proj = ProjectInfo(name="TestProj", description="A test", tasks=[
        Task(text="task one", project="TestProj"),
    ])
    from locus.priorities import sync_project_to_obsidian, projects_dir
...
def test_sync_all_projects(tmp_path):
    ...
    from locus.priorities import sync_all_projects_to_obsidian, projects_dir
