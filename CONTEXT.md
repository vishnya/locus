...
| File | Purpose |
|------|---------|
| `locus/priorities.py` | Parser/writer for PRIORITIES.md (Task, ProjectInfo, Priorities dataclasses) |
| `locus/cli.py` | Argparse entrypoint (`lc` command) |
| `locus/commands/focus.py` | Core workflow: activate, done, bump, park, progress |
| `locus/commands/priority.py` | Task/project add commands |
| `locus/session_status.py` | Cross-device session sync (git-tracked JSON) |
...
- `~/Library/LaunchAgents/com.locus.plist`
...
- Server logs: `tail -f /tmp/locus.log`
- Check server: `launchctl list | grep locus`
- Restart server: `launchctl kickstart -k gui/$UID/com.locus`
- Port conflict: `lsof -ti:5790 | xargs kill -9`
- Host alias requires: `echo '127.0.0.1 locus' | sudo tee -a /etc/hosts`
