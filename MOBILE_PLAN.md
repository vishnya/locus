...
**Goal:** Full Locus features on iPhone, synced with the Mac.
...
### Cons
- **Mac must be on and awake.** If your MacBook sleeps, Locus is unreachable.
...
3. Create a tunnel: `cloudflared tunnel create locus`
...
### Pros
- Custom domain (e.g., `locus.yourdomain.com`).
...
**Concept:** Deploy Locus as-is on a persistent VM. No serverless rewrite needed. Replace file storage with a persistent volume.
...
Build a Locus plugin that renders the board UI inside Obsidian, parsing PRIORITIES.md.
...
- No background sync
- 50MB cache limit (fine for Locus)
...
Mac (always-on, power adapter):
  - Locus server on 0.0.0.0:5790
  - Tailscale installed
  - Obsidian with Sync enabled

iPhone:
  - Tailscale installed
  - Locus PWA on home screen (via Tailscale IP)
  - Obsidian Mobile with Sync (fallback read access)

When Mac is reachable:
  -> Use Locus PWA over Tailscale (full features, chat, drag-and-drop)
...
1. **Tailscale is the foundation.** Full Locus features on your phone with one line of code change. WireGuard security, no auth to build, free. This alone gets you 95% of what you want.

2. **PWA polish is cheap.** 2-3 hours makes Locus feel native on iOS -- full-screen, home screen icon, cached assets. Worth doing regardless.
...
**Why not Vercel?** It fights the architecture. Locus is local-first -- a single markdown file in Obsidian. Serverless + database is a 2-4 week rewrite that destroys what makes Locus elegant. You'd lose Obsidian editing, wikilinks, and simplicity. Chat streaming breaks on the Hobby plan ($0), and Pro is $20/month. Wrong tool for this job.
...
**Why not Cloudflare Tunnel?** Solid Tailscale alternative, especially with a custom domain. But for personal iPhone use, Tailscale is simpler (no auth config, no DNS). CF Tunnel is the upgrade if you want to share Locus with others.
